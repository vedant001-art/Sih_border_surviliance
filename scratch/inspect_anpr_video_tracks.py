...
for f_idx in range(150):
    ...
    for d in dets:
        ...
        if tid not in tracks:
            tracks[tid] = {"class": cname, "frames": 0, "first_f": f_idx, "last_f": f_idx, "first_bbox": bbox, "last_bbox": bbox}
        tracks[tid]["frames"] += 1
        tracks[tid]["last_f"] = f_idx
        tracks[tid]["last_bbox"] = bbox

cap.release()

print("\n--- Tracks in ANPR Video ---")
for tid, info in tracks.items():
    dx = abs(info["last_bbox"][0] - info["first_bbox"][0])
    dy = abs(info["last_bbox"][1] - info["first_bbox"][1])
    is_stationary = (dx < 5 and dy < 5 and info["frames"] > 5)
